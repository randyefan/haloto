...
class UpcomingMaintenanceCell: ASCellNode{
    ...
    override func layoutSpecThatFits(_ constrainedSize: ASSizeRange) -> ASLayoutSpec {
        let stack = ASStackLayoutSpec(direction: .vertical, spacing: 5, justifyContent: .start, alignItems: .start, children: [countUpcomingMaintenanceNode, upcomingMaintenanceNode])
        
//        let inset = ASInsetLayoutSpec(insets: UIEdgeInsets(top: .infinity, left: .infinity, bottom: .infinity, right: .infinity), child: stack)
        
        let backgroundInset = ASBackgroundLayoutSpec(child: stack, background: backgroundNode)
        
        return ASStackLayoutSpec(direction: .horizontal, spacing: 5, justifyContent: .start, alignItems: .start, children: [backgroundInset, upcomingMaintenanceDescription])
    }
}
